...
private fun solution2(): Int {
    val dividerPackets = listOf("[[2]]", "[[6]]")
        .map {
            packetParser(it).getOrThrow()
        }
    val (first, second) = deserializedInput().flatMap { listOf(it.first, it.second) }
        .plus(dividerPackets)
        .sorted()
        .let {
            (it.indexOf(dividerPackets[0]) + 1) to (it.indexOf(dividerPackets[1]) + 1)
        }
    return first * second
}

private fun solution1(): Int {
    return deserializedInput().map {
...
private fun deserializedInput() = input.split("\n\n").map { pairs ->
    val (first, second) = pairs.split("\n")
        .map {
            (packetParser(it) as ParseResult.Ok<Packet.Array>).value
        }
    first to second
}
